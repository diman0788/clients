...
export const createClientItem = (data) => {
    ...
    nameLink.classList.add('clients__link');
    ...
    function locationHashChanged () {
        if (location.hash === `#${data.id.substr(-6)}`) {
            setTimeout(() => {
            document.body.append(editeClient.editeModal);
            }, 1000);
            console.log('HI!')
    ...
    nameLink.href = `#${data.id.substr(-6)}`;
    ...
    nameLink.append(clientLastName, clientName, clientSurname);
    clientFullName.append(nameLink);
